...
if run_analysis:
    ...
    with col_a:
        ...
        # 显示大号评分
        st.markdown(
            f"<h1 style='text-align: center; color: {'red' if fraud_score > 850 else 'orange' if fraud_score > 600 else 'green'};'>{fraud_score:.0f} / 1000</h1>",
    ...
    st.markdown("---")
    st.caption(
        "💡 **分析说明**：绿色虚线标记了每个听觉刺激的呈现时刻。P300是刺激后约300ms出现的正向波峰，其振幅和潜伏期是评估脑功能状态的核心指标。")
